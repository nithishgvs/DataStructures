...
public class ShortestPathWeightedWithMinEdges {
	...
	public static Map<Integer, DistanceEdgeInfo> buildDistanceTable(Graph graph, int source) {
		...
		while (!queue.isEmpty()) {
			...
			for (Integer neighbour : graph.getAdjacentVertices(currentVertexInfo.getVertexId())) {
				// Get the distance and number of edges from the current vertex
				// to the neighbour.
				...
				if (neighbourDistance > distance
						|| ((neighbourDistance == distance) && (distanceTable.get(neighbour).getNumEdges() > edges))) {

					// Update the distance table for the neighbour with the new
					// information.
					distanceTable.get(neighbour).setInfo(currentVertexInfo.getVertexId(), distance, edges);
					...
					// Set up the updated neigbour vertex info with the new
					// distance
					// and number of edges.
	...
	// Vertex Info for Priority Queue
